...
import { User, UserCourse } from 'src/store/models';
...
async function _fetchUserCourses(user_id: number): Promise<Array<UserCourse>> {
	const response = await axios.get(`${process.env.VUE_ROUTER_BASE ?? ''}api/users/${user_id}/courses`);
...
async function _fetchUsers(course_id: number): Promise<Array<User>> {
	const response = await axios.get(`${process.env.VUE_ROUTER_BASE ?? ''}api/courses/${course_id}/users`);
	// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
