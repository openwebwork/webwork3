...
export default defineComponent({
	...
	setup() {
		...
		watch(() => route.params.course_name,
			(course_name) =>  {
				void store.dispatch('session/setCourse', { course_name, course_id: route.params.course_id });
			}
		);

		void store.dispatch('user/fetchUserCourses', store.state.session.user.user_id);

		return {
			student_courses: computed(
				() => store.state.user.user_courses.filter((user: UserCourse) => user.role === 'student')),
			instructor_courses: computed(
				() => store.state.user.user_courses.filter((user: UserCourse) => user.role === 'instructor')),
