<template>
  <div class="row">
    <div v-for="day in week" :key="day" class="col day" :class="background(day)">
			<span class="date">{{ shortDay(day) }}</span> </div>
<!--      <draggable
...
export default defineComponent({
	...
	setup(props) {
		...
		return {
			week: computed( (): Array<Date> => [0,1,2,3,4,5,6].map( (num) => date.addToDate(first_day_of_week,{days: num}) ) ),
			shortDay: (day: Date) => day.getDate() === 1 ? date.formatDate(day,'MMM D') : date.formatDate(day,'D'),
			background: (day: Date) => day.getDate() === 1 ? 'firstday' : ( date.isSameDate(day,new Date(),'day') ? 'today' : '' )
		}
	}
  // private drag = false;
