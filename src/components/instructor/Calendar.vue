...
import CalendarRow from '../common/CalendarComponents/CalendarRow.vue';
...
export default defineComponent({
	...
	setup() {
		...
		const first_day: Ref<Date> = ref(date.subtractFromDate(today,{days: today.getDay()}));  // first day of the calendar
		return {
			first_day,
			calendar_days: computed( () => [0,1,2,3,4].map( (num) => date.addToDate(first_day.value,{days: 7*num}) ) ),
