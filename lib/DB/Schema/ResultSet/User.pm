...
=item * C<user_info>, a hashref of the form C<{ user_id => 1 }> or C<{ login => "username" }>.
...
An arrayref of Users (as hashrefs) or an arrayref of C<DBIx::Class::ResultSet::User>
...
=item - C<login>, the login of an existing user.
...
=item - C<login>, the login of a user (required)
...
=item * If both the login and course name is not included, an error will be thrown.

=item * If the course doesn't exist, an error will be thrown

=item * If the user's login already exists as a global user, an error will be thrown.
...
=item * If the login field is not defined, an error will be thrown.
...
=item - C<login>, the login of a user
