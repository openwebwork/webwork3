...
generate-ww3-pg-pod.pl - Convert WeBWorK3 and PG POD into HTML form.
...
generate-ww-pg-pod.pl [options]
 ...
 Options:
   -w|--webwork3-root    Directory containing a git clone of webwork3.
                         If this option is not set, then the environment
                         variable $WEBWORK3_ROOT will be used if it is set.
...
Note that at least one of the options --webwork3-root or --pg-root must be provided
(or there is nothing to do!).

=head1 DESCRIPTION
 
Convert WeBWorK3 and PG POD into HTML form.
...
my ($webwork3_root, $pg_root, $output_dir, $base_url);
my $verbose = 0;
GetOptions(
	'w|webwork-root=s' => \$webwork3_root,
	'p|pg-root=s'      => \$pg_root,
...
$webwork3_root = $ENV{WEBWORK3_ROOT} if !$webwork3_root;
$pg_root       = $ENV{PG_ROOT}       if !$pg_root;

pod2usage(2) unless (($webwork3_root || $pg_root) && $output_dir);
...
for my $dir ($webwork3_root, $pg_root) {
...
sub process_dir {
	my $source_dir = shift;
	return unless $source_dir =~ /\/webwork3$/ || $source_dir =~ /\/pg$/;

	my $dest_dir = $source_dir;
	$dest_dir =~ s/^$webwork3_root/$output_dir\/webwork3/ if ($source_dir =~ /\/webwork3$/);
