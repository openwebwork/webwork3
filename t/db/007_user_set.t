...
dbSubtest 'course users' => sub ($schema) {
	...
	# FIXME: The getAllUserSets method should not exist and should not be tested.
	# Get all user sets for all courses.
	...
	# sort both arrays
	@all_user_sets =
		sort { $a->{course_name} cmp $b->{course_name} || $a->{set_name} cmp $b->{set_name} } @all_user_sets;
	...
	my @user_sets_for_one_user =
		grep { $_->{course_name} eq 'Precalculus' && $_->{username} eq 'homer' } @all_user_sets;

	my @user_sets_from_db =
		$user_set_rs->getUserSetsForUser(info => { course_name => $course->course_name, username => 'homer' });
	...
	@merged_sets_from_db = $user_set_rs->getUserSetsForUser(
		info   => { course_name => $course->course_name, username => 'homer' },
		merged => 1
	);
	...
	my @user_sets_for_one_set =
		grep { $_->{course_name} eq 'Precalculus' && $_->{set_name} eq 'HW #1' } @all_user_sets;

	@user_sets_from_db =
		$user_set_rs->getUserSetsForSet(info => { course_name => 'Precalculus', set_name => 'HW #1' });
	...
	is(
		dies {
			$user_set_rs->getUserSetsForUser(
				info => { course_name => 'Precalculus', username => 'non_existent_user' });
		},
	...
	my $info     = { username => 'homer', course_name => 'Precalculus', set_name => 'HW #1' };
	my $user_set = $user_set_rs->getUserSet(info => $info);

	my $user_set_from_json = clone(
		(
			grep {
	...
	is($user_set, $user_set_from_json, 'getUserSet: get a user set from a course');

	# Get a merged UserSet

	my $merged_set_from_json = clone(
		(
			grep {
				$_->{course_name} eq 'Precalculus'
					&& $_->{username} eq $info->{username}
	...
	is($merged_set, $merged_set_from_json, 'getUserSet: get a merged set from a course');
	...
	# Add a user set
	my $new_info = { username => 'otto', course_name => 'Precalculus', set_name => 'HW #1' };
	...
	my $hw_set1 =
		clone((grep { $_->{course_name} eq 'Precalculus' && $_->{set_name} eq 'HW #1' } @hw_sets)[0]);

	$hw_set1->{username} = 'frink';

	my $new_merged_set = $user_set_rs->addUserSet(
		params => { course_name => 'Precalculus', set_name => 'HW #1', username => 'frink' },
		merged => 1
	);
	...
	my $new_user_params2 =
		{ username => 'ralph', course_name => 'Arithmetic', set_name => 'HW #3', set_dates => {} };
	...
	my $otto_set_info2 =
		{ username => 'otto', course_name => 'Precalculus', set_name => 'HW #2', set_version => 1 };

	# Add a user set with valid params.
	my $user_set2 = $user_set_rs->addUserSet(
		params => {
			%$otto_set_info2, set_params => { description => 'This is the description for HW #2' }
		}
	);
	removeIDs($user_set2);
	...
	my $set_dates4 = { open => 1, reduced_scoring => 800, due => 900, answer => 1000 };

	my $otto_set_info4 =
		{ username => 'otto', course_name => 'Precalculus', set_name => 'HW #5', set_version => 1 };

	my $otto_set_info5 = { username => 'otto', course_name => 'Precalculus', set_name => 'HW #6' };
	...
	my $hw4 = $problem_set_rs->getProblemSet(info => { course_name => 'Precalculus', set_name => 'HW #4' });
	...
	my $otto_quiz_info = { course_name => 'Precalculus', set_name => 'Quiz #1', username => 'otto' };
	...
	my $user_set_to_merge = $user_set_rs->addUserSet(
		params => { %$otto_quiz_info, set_dates => $new_dates },
		merged => 1
	);
	...
	# This set has a due date before the reduced scoring date.
	is(
		dies {
			$user_set_rs->addUserSet(
				params => { %$otto_set_info5, set_dates => { due => 1609595640 } },
				merged => 1
			);
		},
		check_isa('DB::Exception::ImproperDateOrder'),
	...
	is(
		dies {
			$user_set_rs->addUserSet(
				params => { %$otto_set_info5, set_params => { hide_hint => 1 } },
				merged => 1
			);
		},
	...
	my $updated_user_set2 = $user_set_rs->updateUserSet(
		info   => $otto_quiz_info,
		params => { set_params => { problem_randorder => true } }
	);
	...
	is(
		dies {
			$user_set_rs->updateUserSet(
				info   => $otto_quiz_info,
				params => { set_dates => { open => 1, closed => 2 } }
			);
		},
		check_isa('DB::Exception::InvalidField'),
	...
	is(
		dies {
			$user_set_rs->updateUserSet(
				info   => $otto_set_info5,
				params => { set_params => { hide_hint => true } }
			);
		},
