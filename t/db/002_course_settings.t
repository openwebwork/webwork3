...
my $updated_general_setting = {
	general => {
		course_description => "This is my new course description"
	}
};

my $updated_course_settings = $course_rs->updateCourseSettings(
	$new_course_info, $updated_general_setting
);

my $current_course_values = mergeCourseSettings($default_course_values,$updated_general_setting);

# dd "printing course_description";
# dd $updated_course_settings->{general};
# dd $current_course_values->{general};
...
is_deeply(
	...
	"course_settings: updated general setting"
);

# update another general setting

$updated_general_setting = {
	general => {
		hardcopy_theme => "One Column"
	}
};
...
is_deeply(
	$current_course_values,
	$updated_course_settings,
	"course_settings: updated another general setting"
);

# set a single course setting in Optional Modules

my $updated_optional_setting = {
	optional => {
		enable_show_me_another => 1
	}
};
$updated_course_settings = $course_rs->updateCourseSettings(
	$new_course_info, $updated_optional_setting
);
...
is_deeply(
	$current_course_values,
	$updated_course_settings,
	"course_settings: updated optional setting"
);


# set a single course setting in problem_set

my $updated_problem_set_setting = {
	problem_set => {
		 time_assign_due => "11:52"
	}
};
$updated_course_settings = $course_rs->updateCourseSettings(
	$new_course_info, $updated_problem_set_setting
);
...
is_deeply(
	$current_course_values,
	$updated_course_settings,
	"course_settings: updated problem set setting"
);

# set a single course setting in problem

my $updated_problem_setting = {
	problem => {
		 display_mode => "images"
	}
};
$updated_course_settings = $course_rs->updateCourseSettings(
	$new_course_info, $updated_problem_setting
);
...
is_deeply(
	$current_course_values,
	$updated_course_settings,
	"course_settings: updated problem setting"
);

# make sure that an nonexistant setting throws an exception

my $undefined_problem_setting = {
	general => {
		non_existent_setting => 1
	}
};

throws_ok {
	$course_rs->updateCourseSettings(
		$new_course_info, $undefined_problem_setting
	);
...
my $invalid_list_option = {
	general => {
		hardcopy_theme => 'default'
	}
};

$course_rs->updateCourseSettings(
	$new_course_info, $invalid_list_option
);
