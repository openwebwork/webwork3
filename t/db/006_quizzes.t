...
is($quiz_from_db, $quiz_from_csv[0], 'getQuiz: get one quiz from a single course');
...
is(
	dies {
		$problem_set_rs->addProblemSet(params => { course_name => 'nonexistent course', set_name => 'Quiz #1' });
	},
	check_isa('DB::Exception::CourseNotFound'),
...
is(
	dies {
		$problem_set_rs->addProblemSet(params =>
				{ course_name => 'Precalculus', set_type => 'QUIZ', set_name => 'Quiz #99', nonexistent_field => 1, })
	},
...
is(
	dies { $problem_set_rs->deleteProblemSet(info => { course_name => 'Precalculus', set_name => 'Quiz #999' }); },
	check_isa('DB::Exception::SetNotInCourse'),
...
# Try to delete from a non-existent set in a course.
is(
	dies { $problem_set_rs->deleteProblemSet(info => { course_name => 'Precalculus', set_id => 99999 }); },
	check_isa('DB::Exception::SetNotInCourse'),
...
is(\@precalc_quizzes_from_db, \@precalc_quizzes, 'check: ensure that the quizzes have been restored.');
