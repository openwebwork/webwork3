...
use BuildDB qw/loadPermissions addCourses addUsers addProblemPools/;
...
mojoDBSubtest 'user problem routes for review sets' => sub ($t, $schema) {
	...
	addProblemPools($schema, $t->app->home);
	...
	# Load arithmetic problem pools and pool problems from the JSON file.
	my (@arith_problem_pools, @arith_pool_problems);
	for my $course_info (@{ decode_json($t->app->home->child('t/db/sample_data/pool_problems.json')->slurp) }) {
		next unless $course_info->{course_name} eq 'Arithmetic';
		for my $problem_pool_info (@{ $course_info->{pools} }) {
			push(@arith_problem_pools, { pool_name => $problem_pool_info->{pool_name} });
			for my $pool_problem (@{ $problem_pool_info->{pool_problems} }) {
				push(@arith_pool_problems, { %$pool_problem, pool_name => $problem_pool_info->{pool_name} });
			}
		}
	}
	...
	$t->put_ok(
		...
		->json_is('/pool_name' => 'adding decimals');
	...
	$t->get_ok(
		"/webwork3/api/courses/4/pools/$prob_pool1->{problem_pool_id}/problems/$pool_prob1->{pool_problem_id}")
		->status_is(200)->content_type_is('application/json;charset=UTF-8')
	...
	$t->put_ok(
		"/webwork3/api/courses/4/pools/$added_problem_pool->{problem_pool_id}/"
			. "problems/$new_pool_problem->{pool_problem_id}" => json => {
	...
	$t->put_ok("/webwork3/api/courses/4/pools/$added_problem_pool->{problem_pool_id}" => json =>
			{ pool_name => 'another name' })->status_is(403);
	...
	$t->put_ok("/webwork3/api/courses/4/pools/$added_problem_pool->{problem_pool_id}/"
			. "problems/$new_pool_problem->{pool_problem_id}" => json => { params => { library_id => 8932 } })
		->status_is(403);
	$t->delete_ok("/webwork3/api/courses/4/pools/$added_problem_pool->{problem_pool_id}/"
			. "problems/$new_pool_problem->{pool_problem_id}")->status_is(403);
	...
	$t->delete_ok("/webwork3/api/courses/4/pools/$added_problem_pool->{problem_pool_id}/"
			. "problems/$new_pool_problem->{pool_problem_id}")->status_is(200)
		->content_type_is('application/json;charset=UTF-8')->json_is('/params/library_id' => 8932);
