...
mojoDBSubtest 'course users routes' => sub ($t, $schema) {
	...
	$t->get_ok("/webwork3/api/courses/4/users/$user_id")->status_is(200)
		->content_type_is('application/json;charset=UTF-8')->json_is('/user_id' => $user_id)
	...
	my $course_user_params = {
		user_id            => $new_user_id,
		role               => 'student',
		course_user_params => { comment => "I love my big sister", useMathQuill => true }
	};
	...
	# Try to get a user that is not in a course.
	$t->get_ok("/webwork3/api/courses/4/users/6")->status_is(500, 'status for exception')
	...
	$t->put_ok("/webwork3/api/courses/4/users/6" => json => { recitation => '2' })
	...
	$t->delete_ok("/webwork3/api/courses/4/users/6")->status_is(500, 'status for exception')
		->content_type_is('application/json;charset=UTF-8')->json_is('/exception' => 'DB::Exception::UserNotInCourse');
	...
	$t->post_ok('/webwork3/api/courses/3/users' => json => { user_id => $new_user_id, role => 'student' })
		->status_is(403)->content_type_is('application/json;charset=UTF-8');

	$t->put_ok('/webwork3/api/courses/3/users/' . $new_user_id => json => { recitation => 4 })->status_is(403)
		->content_type_is('application/json;charset=UTF-8');
