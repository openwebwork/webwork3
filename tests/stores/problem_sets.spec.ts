...
describe('Problem Set store tests', () => {
	...
	describe('fetching set problems for a course', () => {
		test('fetching problems for a course.', async () => {
			...
			precalc_problems_from_csv = precalc_probs.map(prob => parseProblem(prob as ParseableSetProblem, 'Set'));
			expect(cleanIDs(problem_set_store.set_problems))
	...
	describe('CRUD tests for set problems', () => {
		...
		test('Add a set problem to a set', async () => {
			...
			const probs = problem_set_store.findSetProblems({ set_name: 'HW #1' });
			...
			new_problem = await problem_set_store.addSetProblem({ set_id: hw1?.set_id ?? 0, problem: lib_prob });
		...
		test('Update a set problem', async () => {
			...
			const problem_from_server = await problem_set_store.updateSetProblem({
		...
		test('Delete a set problem', async () => {
			const problem_set_store = useProblemSetStore();
			const problems = problem_set_store.findSetProblems({ set_name: 'HW #1' });
			const deleted_problem = await problem_set_store.deleteSetProblem(new_problem);
			expect(deleted_problem).toStrictEqual(updated_problem);
			expect(problem_set_store.findSetProblems({ set_name: 'HW #1' }).length).toBe(problems.length - 1);
		});
	});
